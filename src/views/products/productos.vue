    ...
    <div id="Categorias">
          ...
          <div class="options-cont" @click="closeSelect()">
            ...
            <div class="tbl_header dflx">
                <div class="tblh tblrad name">
                    <p>NOMBRE</p>
                </div>
                 <div class="tblh ">
                    <p>CATEGORIA / SUBCAT.</p>
                </div>
                 <div class="tblh ">
                    <p>DESCRIPCIÓN</p>
                </div>
            ...
            <div tbl_content>
                ...
                <div  v-if="data.length>0">
                    <div class="tbl_items dflx" v-for="(item, key) in filteredItems" :key="key">
                        <div class="tbl_item name">
                            <p >{{item.name}}
                            <span v-if="item.product_atributo_valores.length > 0">{{" - "+item.product_atributo_valores[0].valor+" "+item.product_atributo_valores[0].atributo}}</span></p>
                        </div>
                        <div class="tbl_item ">
                            <p>{{item.category_name}} <span v-if="item.subcategorias[0]">/ {{item.subcategorias[0].subcategory_name}}</span></p>
                        </div>
                        <div class="tbl_item ">
                            <p>{{item.descr}}</p>
                        </div>
          ...
          <div  id="navigate-buttons" class="mlf">
           ...
           <div v-if="viewModal" class="modal_view">
            ...
            <h3 >¿Estás seguro de  <span>eliminar <br> este producto?</span></h3>
                </div>
...
import AddPrd from './addPrd.vue'
import editPrd from './editPrd.vue'
export default {
  name: 'Categorias',
  components:{
   AddPrd,
       ...
       ...mapActions('product', ['getAllInfoPrd']),
         ...
         ...mapActions('product', ['setAddedPrd']),
           ...
           changeView(option, item){
             this.item=item
             this.setAddedPrd(item)
             this.setHistoryOptionPrd(option)
         },
 ...
 includesItem(search, name) {
        ...
        eliminar: async function(){
            ...
            let result = await this.$store.dispatch("product/deleteItemPrd", {option: 'product', id:id});
...
.tbl_item.name,.tblh.name{    width: 15vw;}
      .tblh{
            width: 10.78125VW;
            height: 2.5VW;
