    ...
    <div id="Tag">
          ...
          <div  id="navigate-buttons" class="mlf2">
           ...
           <div v-if="viewModal" class="modal_view">
            ...
            <h3 >¿Estás seguro de  <span>eliminar <br> esta categoría?</span></h3>
                </div>
...
import AddCtg from './addCtg.vue'
import EditCtg from './editCtg.vue'
export default {
  name: 'Tag',
  components:{
   AddCtg,
  ...
  computed:{
        data() {
           const arr = this.$store.getters["category/data"];
  ...
  methods:{
         ...
         ...mapActions('category', ['setAddedCtg']),
           ...
           changeView(option, item){
             this.item=item
             this.setAddedCtg(item)
             this.setHistoryOptionCtg(option)
         },
        ...
        eliminar: async function(){
            ...
            this.message='';
             ...
             let result = await this.$store.dispatch("category/deleteItemCtg", {option: 'category', id:id});
