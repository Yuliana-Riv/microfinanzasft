...
import AddTpe from './addTpe.vue'
import EditTpe from './editTpe.vue'
export default {
  name: 'Type',
  components:{
   AddTpe,
  ...
  computed:{
        data() {
           const arr = this.$store.getters["type/data"];
  ...
  methods:{
         ...
         ...mapActions('type', ['setAddedTpe']),
           ...
           changeView(option, item){
             this.item=item
             this.setAddedTpe(item)
             this.setHistoryOptionTpe(option)
         },
        ...
        eliminar: async function(){
            ...
            this.message='';
             ...
             let result = await this.$store.dispatch("type/deleteItemTpe", {option: 'type', id:id});
