...
export default {
    ...
    getters: {
        ...
        device(state) {
            return state.device_session_id;
        },
        loading_img(state) {
    ...
    mutations: {
        ...
        [SET_SOURCE](state, change){
            state.source_id = change;
        },
        [SET_DEVICE](state, change){
            state.device_session_id = change;
        },
    ...
    actions:{
        setAddedPsl({commit}, payload){
            commit(SET_ADDED, payload);  
        },
        setSourcePsl({commit}, payload){
            commit(SET_SOURCE, payload);  
        },
        setDevicePsl({commit}, payload){
            commit(SET_DEVICE, payload);  
        },

        setHistoryOptionPsl({commit}, payload){
            commit(SET_HISTORY_OPTION, payload);  
        },
        async getAllInfoPsl({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
            //console.log(payload)
        ...
        async getInfoByIdPsl({ commit }, payload) {
            commit(GETTING_INFO);
            try {              
                let token = userService.getToken() 
                let headers =  {'Authorization': token} ;
        ...
        async searchPsl({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
        ...
        async deleteItemPsl({ commit }, payload) { 
        ...
        async editItemPsl({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
        ...
        async addItemPsl({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
